...
try {
    ...
    $pyArgs = @('-3')
    if (-not $py) { $py = (Get-Command python.exe -ErrorAction SilentlyContinue).Source; $pyArgs = @() }
    if (-not $py) { throw '未找到 Python 启动器或 python.exe' }
    $args = @($pyArgs) + @('fusion_gateway.py','--transport','http','--host','0.0.0.0','--port',"$port")
    $p = Start-Process -FilePath $py -ArgumentList $args -WorkingDirectory $root -WindowStyle Hidden -RedirectStandardOutput (Join-Path $state 'gateway.stdout.log') -RedirectStandardError (Join-Path $state 'gateway.stderr.log') -PassThru
    for ($i = 0; $i -lt 50; $i++) {
