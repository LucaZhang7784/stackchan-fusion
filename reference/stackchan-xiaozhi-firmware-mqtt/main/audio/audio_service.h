...
#define MAX_PLAYBACK_TASKS_IN_QUEUE 256  // 推送 PCM 缓冲(15.4s@60ms), 吸收送达突发且不阻塞 MQTT 任务
...
struct AudioTask {
    ...
    uint32_t timestamp;
    // 仅主动推送的首个 PCM 任务使用：由 AudioOutputTask 在真正送入 I2S 前执行一次。
...
public:
    ...
    bool PushPcmToPlaybackQueue(const uint8_t* data, size_t bytes,
                                std::function<void()> on_playback_start = {});
