...
void CustomWakeWord::ParseWakenetModelConfig() {
    ...
    // 唤醒词固定为「阿松」(xiaozhi.me 控制台无法配置, 固件直接覆盖)。
    // 拼音 a song 需在 MultiNet6 词库覆盖范围内; 保留 你好小智 作兜底。
    commands_.clear();
    commands_.push_back({"a song", "阿松", "wake"});
...
void CustomWakeWord::StoreWakeWordData(const std::vector<int16_t>& data) {
    ...
    // Phase 7.1: 竞态修复 - 与编码任务互斥, 防止 Feed 线程 push_back 与迭代并发崩溃
...
void CustomWakeWord::EncodeWakeWordData() {
    ...
    {
        // Phase 7.1: 竞态修复 - 输出队列同样受锁保护(GetWakeWordOpus 会并发弹取)
        std::lock_guard<std::mutex> lock(wake_word_mutex_);
    ...
    wake_word_encode_task_ = xTaskCreateStatic([](void* arg) {
        auto this_ = (CustomWakeWord*)arg;
        {
            ...
            // Phase 7.1: 竞态修复 - 在锁内把输入缓冲整体 swap 出, 锁外编码。
            // 原实现无锁遍历/clear wake_word_pcm_, 与 Feed 线程 push_back 并发,
            // 导致 vector 引用悬垂, in_buffer.erase 触发 LoadProhibited 崩溃。
            std::deque<std::vector<int16_t>> pcm_to_encode;
            {
                std::lock_guard<std::mutex> lock(this_->wake_word_mutex_);
                pcm_to_encode.swap(this_->wake_word_pcm_);
            }
