...
// Phase 7.1: 音频编解码与触屏共享 I2C 总线, 启用/停用前加互斥(50ms 超时, 防优先级翻转死锁)
static bool LockAudioI2c(SemaphoreHandle_t* out_mutex) {
    *out_mutex = Board::GetInstance().GetI2cBusMutex();
    if (*out_mutex == nullptr) return true;
    return xSemaphoreTake(*out_mutex, pdMS_TO_TICKS(50)) == pdTRUE;
}
static void UnlockAudioI2c(SemaphoreHandle_t mutex) {
...
void CoreS3AudioCodec::EnableInput(bool enable) {
    ...
    if (!LockAudioI2c(&i2c_mutex)) {
        ESP_LOGW(TAG, "EnableInput: I2C bus busy, skip");
        return;
    }
    if (enable) {
...
void CoreS3AudioCodec::EnableOutput(bool enable) {
    ...
    SemaphoreHandle_t i2c_mutex = nullptr;
    if (!LockAudioI2c(&i2c_mutex)) {
        ESP_LOGW(TAG, "EnableOutput: I2C bus busy, skip");
        return;
    }
    if (enable) {
