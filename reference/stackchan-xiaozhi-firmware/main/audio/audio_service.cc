...
AudioService::~AudioService() {
    ...
    if (preroll_buf_ != nullptr) {
        free(preroll_buf_);
        preroll_buf_ = nullptr;
    }
...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    audio_processor_->OnVadStateChange([this](bool speaking) {
        ...
        // Phase 7.1: 本地 VAD 检测到语音起点, 立即把预卷(含语音起始段)瞬时送入编码队列
        if (speaking) {
            FlushPrerollToSendQueue();
        }
    ...
    // Phase 7.1: 500ms 预录音环形缓冲, 明确分配在 PSRAM(SPIRAM 不可用时回退内部堆)
    preroll_buf_ = (int16_t*)heap_caps_malloc(kPrerollSamples * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    if (preroll_buf_ == nullptr) {
        preroll_buf_ = (int16_t*)malloc(kPrerollSamples * sizeof(int16_t));
        if (preroll_buf_ != nullptr) {
            ESP_LOGW(TAG, "SPIRAM preroll alloc failed, fallback to internal heap");
        }
    }
    if (preroll_buf_ != nullptr) {
        memset(preroll_buf_, 0, kPrerollSamples * sizeof(int16_t));
        ESP_LOGI(TAG, "Preroll ring buffer ready: %d samples (%dms) @ PSRAM",
                 (int)kPrerollSamples, (int)(kPrerollSamples * 1000 / 16000));
    } else {
        ESP_LOGE(TAG, "Preroll ring buffer alloc failed");
    }
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        if (audio_input_need_warmup_) {
            ...
            // Phase 7.1: 开麦暖机 - 丢弃前 60ms PCM(电涌 Click) + 30ms AFE 静音门槛(防回声/自言自语)
            vTaskDelay(pdMS_TO_TICKS(AUDIO_MIC_OPEN_WARMUP_MS));
            continue;
        }
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        // Phase 7.1 终极修补: EOF(stop) 后需等待解码/播放队列都排空才唤醒回调
        audio_queue_cv_.wait(lock, [this]() {
            return service_stopped_ ||
                   !audio_playback_queue_.empty() ||
                   (playback_eof_ && audio_decode_queue_.empty());
        });
        if (service_stopped_) {
            break;
        }

        // Phase 7.1: 打断软刷新 - 在写 I2S 的唯一任务内清空队列 + 静音帧冲刷 DMA,
        // 不跨线程调用 i2s_channel_disable(避免双工 RX 丢时钟/内核互斥死锁)
        if (abort_requested_) {
            audio_playback_queue_.clear();
            audio_decode_queue_.clear();
            audio_queue_cv_.notify_all();
        ...
        // Phase 7.1 终极修补: 尾音排空 → 通知应用切待机/聆听。
        // 严禁在 stop 时 ClearQueue 或立即切 Idle(会掐尾音/提前亮待机灯)。
        if (playback_eof_ && audio_playback_queue_.empty() && audio_decode_queue_.empty()) {
            playback_eof_ = false;
            lock.unlock();
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            // Phase 7.1: 打断期间丢弃解码, 防止重新填充播放队列造成"停完又播"
            if (abort_requested_) {
                continue;
            }
...
bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    ...
    if (audio_decode_queue_.size() >= MAX_DECODE_PACKETS_IN_QUEUE) {
        if (wait) {
            // 背压最多等 2 秒; 仍满则丢弃(不可无限阻塞 websocket 任务, 否则 task_wdt 会重启设备)
            audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(2000), [this]() {
...
void AudioService::ResetDecoder() {
    ...
    std::unique_lock<std::mutex> decoder_lock(decoder_mutex_);
    if (opus_decoder_ != nullptr) {
        // Phase 7.1: esp_opus_dec_reset 底层即 opus_decoder_ctl(st, OPUS_RESET_STATE)
        // (已对 esp_audio_codec 2.4.1 二进制反汇编核实: struct[0]=OpusDecoder*, 请求码 0x0FBC=4028)
        esp_opus_dec_reset(opus_decoder_);
    }
    decoder_lock.unlock();
    playback_eof_ = false;  // Phase 7.1 终极修补: 新播报/打断清除 EOF, 防止误触发排空回调
...
void AudioService::MarkPlaybackEof() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    playback_eof_ = true;
    audio_queue_cv_.notify_all();
}
...
void AudioService::FlushDmaWithSilence() {
    // 向 DMA 写入 1~2 帧全 0 静音 PCM, 保持双工 RX(麦克风) BCLK 时钟, 不做 channel disable
    if (codec_ == nullptr) {
        return;
    }
    if (!codec_->output_enabled()) {
    ...
    std::vector<int16_t> silence(2 * AUDIO_CODEC_DMA_FRAME_NUM * codec_->output_channels(), 0);
    codec_->OutputData(silence);
}

void AudioService::PushPreroll(const std::vector<int16_t>& pcm) {
    if (preroll_buf_ == nullptr || pcm.empty()) {
        return;
    }
    for (int16_t sample : pcm) {
        preroll_buf_[preroll_pos_] = sample;
        preroll_pos_ = (preroll_pos_ + 1) % kPrerollSamples;
    }
}

void AudioService::FlushPrerollToSendQueue() {
    if (preroll_buf_ == nullptr) {
        return;
    }
    // 按 60ms(960 samples) Opus 帧切分, 瞬时(首帧突发)送入编码队列; 不做 30ms pacing,
    // 避免预卷造成 250ms 墙上时间积压导致实时流永久滞后
    std::vector<int16_t> frame;
    frame.reserve(960);
    size_t idx = preroll_pos_;  // 从最老样本开始
    for (size_t i = 0; i < kPrerollSamples; ++i) {
        frame.push_back(preroll_buf_[idx]);
