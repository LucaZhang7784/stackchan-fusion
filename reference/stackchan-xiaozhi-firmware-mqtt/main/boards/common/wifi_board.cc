...
// Connection timeout in seconds: 20s 覆盖 DHCP 首次 OFFER 丢包后的重试窗口,
// 避免 radio 已连上但 IP 未到(间歇性 DHCP 慢)被误判超时进配网。
static constexpr int CONNECT_TIMEOUT_SEC = 20;
...
static void IncrementFailureCount() {
    ...
    // 2026-08-13: 不再累计失败自动清空已存 SSID —— 偶发 DHCP 慢/丢包导致的超时
    // 误进配网不应抹掉家庭 WiFi; 配网入口(开机单击/待机长按)仍在, 异地配新网
    // 经 SsidManager::AddSsid 追加, 新旧网络并存。
}
...
void WifiBoard::EnterWifiConfigMode() {
    ...
    // 2026-08-12: 不再清空已存 SSID —— 误入配网(开机单击/待机长按)不再抹掉家庭 WiFi;
    // 配网成功经 SsidManager::AddSsid 追加新网络, 新旧 SSID 并存,
