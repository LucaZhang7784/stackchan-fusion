...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        // 只有首帧已提交给音频驱动后才回执；不把 MQTT START 或 PCM 入队误记成起播。
        if (task->on_playback_start) {
            task->on_playback_start();
        }
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (opus_decoder_ != nullptr) {
                // 输出缓冲必须精确等于 60ms@16k mono (1920B): esp_opus_dec_decode 用
                // out_frame.len/2 作为 opus frame_size, 加余量会让 frame_size 非法(1216),
                // 导致每帧只解出部分音频(实测 320B)。16k 直通后无重采样, 无越界风险。
...
bool AudioService::PushPcmToPlaybackQueue(const uint8_t* data, size_t bytes,
                                          std::function<void()> on_playback_start) {
    // 推送链路专用: 16kHz 单声道 µ-law(G.711), 60ms/帧 = 960B/帧。
    // 网关传 µ-law 而非 PCM: 带宽减半(16KB/s), 配合 lwIP 窗口 16KB 后公网
    // RTT(~0.5s)下仍有余量, 根治 TCP 窗口导致的播报卡顿; 不经过 Opus 解码器。
    constexpr size_t kFrameBytes = 16000 * 1 * 60 / 1000;  // 960 (µ-law 1B/采样)
    size_t frames = bytes / kFrameBytes;
    if (frames == 0 || data == nullptr) {
        return false;
    }
    // µ-law -> s16le 查表(ITU G.711), 首次调用时初始化 256 项
    ...
    if (!table_ready) {
        for (int i = 0; i < 256; ++i) {
            ...
            int t = ((u & 0x0F) << 3) + 0x84;
            t <<= (u & 0x70) >> 4;
            ulaw_table[i] = (int16_t)((u & 0x80) ? (0x84 - t) : (t - 0x84));
        }
        table_ready = true;
    }
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    size_t space = MAX_PLAYBACK_TASKS_IN_QUEUE - audio_playback_queue_.size();
    size_t push_n = std::min(frames, space);
    for (size_t i = 0; i < push_n; ++i) {
        auto task = std::make_unique<AudioTask>();
        task->type = kAudioTaskTypeDecodeToPlaybackQueue;
        task->timestamp = 0;
        if (i == 0) {
            task->on_playback_start = std::move(on_playback_start);
        }
        task->pcm.resize(kFrameBytes);
        for (size_t s = 0; s < kFrameBytes; ++s) {
            task->pcm[s] = ulaw_table[data[i * kFrameBytes + s]];
        }
        audio_playback_queue_.push_back(std::move(task));
    }
    audio_queue_cv_.notify_all();
...
void AudioService::ResetDecoder() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    // 注意: 严禁调用 esp_opus_dec_reset()! 预编译库的该接口是"假重置", 会破坏解码器内部
    // 状态(实测首会话 1920B 完整, 之后所有会话 320B=1/6 破音)。改为关闭重开,
    // 保证每次播报使用全新解码器实例。
    {
        std::unique_lock<std::mutex> decoder_lock(decoder_mutex_);
        if (opus_decoder_ != nullptr) {
        ...
        esp_opus_dec_cfg_t opus_dec_cfg = OPUS_DEC_CFG(codec_->output_sample_rate(), OPUS_FRAME_DURATION_MS);
