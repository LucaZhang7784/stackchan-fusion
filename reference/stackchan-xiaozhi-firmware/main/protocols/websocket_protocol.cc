...
bool WebsocketProtocol::IsStale(int max_idle_s) const {
    ...
    return std::chrono::duration_cast<std::chrono::seconds>(now - last_incoming_time_).count() > max_idle_s;
}
...
void WebsocketProtocol::CloseAudioChannel(bool send_goodbye) {
    ...
    // Phase 7.1: 异步安全取消 - 先优雅关闭(通知后台 TCP/接收线程退出), 再释放句柄,
    // 严禁直接 delete 导致 Use-After-Free 崩机
...
bool WebsocketProtocol::OpenAudioChannel(bool silent) {
    std::lock_guard<std::mutex> lock(connect_mutex_);
    if (websocket_ != nullptr && websocket_->IsConnected()) {
        return true;
    }
    ...
    audio_aborted_ = false;

    ESP_LOGI(TAG, "OpenAudioChannel: step1 CreateWebSocket");
    ...
    // Phase 7.1: 预卷首帧突发包可达 ~3KB, 接收缓冲 2048 太小会被截断导致服务端断连
    websocket_->SetReceiveBufferSize(4096);
    ESP_LOGI(TAG, "OpenAudioChannel: step2 SetHeader+Connect (url=%s)", url.c_str());
    ...
    websocket_->OnData([this](const char* data, size_t len, bool binary) {
        if (binary) {
            // Phase 7.1: 打断后丢弃服务端仍在推送的音频, 防止"停完又播"
            if (audio_aborted_) {
